...
const server = http.createServer(async (req, res) => {
    ...
    if (req.url.startsWith('/api/')) {
        ...
        if (req.url.startsWith('/api/auth/firebase') && req.method === 'POST') {
            let body = '';
            req.on('data', chunk => body += chunk);
            req.on('end', async () => {
                const { token, phone, name, email } = JSON.parse(body);
                // In production: verify 'token' using firebase-admin SDK
                
                const cleanPhone = normalizePhone(phone);
                let phoneWithPlus = cleanPhone;
                if (phoneWithPlus && !phoneWithPlus.startsWith('+')) phoneWithPlus = '+' + phoneWithPlus;
                
                const filter = phoneWithPlus ? { phone: phoneWithPlus } : { email: email };
                ...
                if (!customer) {
                    ...
                    await dbRequest('updateOne', 'customers', { 
